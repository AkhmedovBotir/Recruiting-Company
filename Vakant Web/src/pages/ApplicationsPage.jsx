...
import { APPLICATION_STATUS, INTERVIEW_STATUS_COLORS, INTERVIEW_STATUS_LABELS, INTERVIEW_RESULT_COLORS, INTERVIEW_RESULT_LABELS } from '../utils/constants';
...
const ApplicationsPage = () => {
  ...
  const [activeTab, setActiveTab] = useState('applications'); // 'applications' or 'interviews'
  ...
  const [interviewPagination, setInterviewPagination] = useState(null);
  ...
  }, [isAuthenticated, filters.page, filters.status, interviewFilters.page, interviewFilters.status, interviewFilters.result, activeTab]);
  ...
  const handleInterviewFilterChange = (key, value) => {
    setInterviewFilters({ ...interviewFilters, [key]: value, page: 1 });
  };
  ...
  const fetchInterviews = async () => {
    ...
    try {
      const params = {
        page: interviewFilters.page,
        limit: interviewFilters.limit,
      };
      if (interviewFilters.status) params.status = interviewFilters.status;
      if (interviewFilters.result) params.result = interviewFilters.result;

      const response = await api.getMyInterviews(params);
      if (response.success) {
        setInterviews(response.data.interviews);
        setInterviewPagination(response.data.pagination);
      }
    } catch (err) {
            ...
            >
              ...
              <div className="bg-white rounded-lg shadow-md p-4 md:p-6 mb-6">
                ...
                >
                  ...
                  <option value={APPLICATION_STATUS.PASSED}>
                    Intervyudan o'tdi
                  </option>
                  <option value={APPLICATION_STATUS.FAILED}>
                    Intervyudan o'tmadi
                  </option>
          ...
          <>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => handleInterviewFilterChange('status', e.target.value)}
                  ...
                  <div>
                    ...
                    <select
                      value={interviewFilters.result}
                      onChange={(e) => handleInterviewFilterChange('result', e.target.value)}
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                    >
            ...
            {loadingInterviews ? (
              ...
              <>
                ...
                {/* Pagination */}
                {interviewPagination && interviewPagination.pages > 1 && (
                  <div className="flex justify-center items-center space-x-2">
                    <button
                      onClick={() =>
                        setInterviewFilters({ ...interviewFilters, page: interviewFilters.page - 1 })
                      }
                      disabled={interviewFilters.page === 1}
                      className="px-4 py-2 border rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-100"
                    >
                      Oldingi
                    </button>
                    <span className="px-4 py-2">
                      {interviewFilters.page} / {interviewPagination.pages}
                    </span>
                    <button
                      onClick={() =>
                        setInterviewFilters({ ...interviewFilters, page: interviewFilters.page + 1 })
                      }
                      disabled={interviewFilters.page === interviewPagination.pages}
                      className="px-4 py-2 border rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-100"
                    >
                      Keyingi
                    </button>
